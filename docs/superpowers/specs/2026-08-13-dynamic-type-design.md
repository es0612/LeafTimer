...
| 3 | 保証する文字サイズ | **AX5 (最大) まで**レイアウトが崩れないことを保証 (HistoryView の 7 日グラフのみ例外。「設計 5」内の「例外」節を参照) |
...
- [ ] 検証マトリクス 13 枚を取得し、AX5 で全画面が崩れず読めることを確認 (HistoryView の 7 日グラフのみ `accessibility3` 上限までの保証。「設計 5」内の「例外」節を参照)
